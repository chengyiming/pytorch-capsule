...
class firstOctConv(nn.Module):
    def __init__(self, settings, ch_in, ch_out, kernel=(1,1), pad=(0,0), stride=(1,1)):
        ...
        self.hh_conv = nn.Conv2d(ch_in, hf_ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)
        self.hl_pool = nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), ceil_mode=True)
        self.hl_conv = nn.Conv2d(ch_in, lf_ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)

    def forward(self, x):
        if self.stride== (2, 2):
            x = self.downsample(x)

        high_high = self.hh_conv(x)
        high_low = self.hl_pool(x)
        high_low = self.hl_conv(high_low)
        out_h = high_high
        out_l = high_low
        return out_h, out_l
...
class lastOctConv(nn.Module):
    def __init__(self, settings, ch_in, ch_out, kernel=(1,1), pad=(0,0), stride=(1,1)):
        ...
        self.hf_ch_out = ch_out

        if stride == (2, 2):
            self.downsample = nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), ceil_mode=True)
        self.hh_conv = nn.Conv2d(hf_ch_in, ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)
        self.lh_upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.lh_conv = nn.Conv2d(lf_ch_in, ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)

    def forward(self, hf_data, lf_data):
        if self.stride== (2, 2):
            hf_data = self.downsample(hf_data)
            lf_data = self.downsample(lf_data)

        high_high = self.hh_conv(hf_data)
        low_high = self.lh_conv(lf_data)
        low_high = self.lh_upsample(low_high)
        out_h = low_high + high_high
        return out_h
...
class OctConv(nn.Module):
    def __init__(self, settings, ch_in, ch_out, kernel=(1,1), pad=(0,0), stride=(1,1)):
        ...
        if stride == (2, 2):
            self.downsample = nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), ceil_mode=True)
        self.hh_conv = nn.Conv2d(hf_ch_in, hf_ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)
        self.hl_pool = nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), ceil_mode=True)
        self.hl_conv = nn.Conv2d(hf_ch_in, lf_ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)

        self.lh_conv = nn.Conv2d(lf_ch_in, hf_ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)
        self.lh_upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.ll_conv = nn.Conv2d(lf_ch_in, lf_ch_out, kernel_size=kernel, stride=(1,1), padding=pad, bias=False)
    ...
    def forward(self, hf_data, lf_data):
        ...
        low_low = self.ll_conv(lf_data)
        high_low = self.hl_pool(hf_data)
        high_low = self.hl_conv(high_low)

        out_h = low_high + high_high
        out_l = low_low + high_low
